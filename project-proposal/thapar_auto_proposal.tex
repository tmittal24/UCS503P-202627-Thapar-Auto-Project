...
\vspace*{1.5cm}
\includegraphics[width=3.2cm]{thapar_logo.png}

\vspace{2cm}
{\bfseries PROJECT PROPOSAL}\par

\vspace{0.8cm}
{\LARGE\bfseries Campus Ride: A Campus Auto-Booking\\[0.2em] and Ride-Pooling Platform \par}

\vspace{0.6cm}
{\large Software Engineering (UCS503) \par}
...
\vspace{2.5cm}
\begin{tabular}{ll}
\textbf{Team Members} & \hspace{1cm}\textbf{Roll Number} \\[0.5em]
Arpita Girdhar & \hspace{1cm}1024030778 \\
Trijal Mittal & \hspace{1cm}1024030784 \\
Tushit Kumar & \hspace{1cm}1024031079 \\
\end{tabular}
...
\section*{Abstract}
Shared auto-rickshaws are the primary way students move around the Thapar campus, but booking one today is entirely informal: students wait at fixed points hoping a free seat passes by, drivers have no visibility into demand elsewhere, and fares are settled by negotiation rather than a fixed rate. This proposal presents Campus Ride, a two-sided mobile application built in Flutter for Android and the web that digitizes this process end to end. Students select a pickup and drop stop and are automatically pooled onto an auto already en route in the right direction, or matched to the next auto in a fair, cyclic idle queue if no such match exists; drivers get a simple dashboard showing their queue position or live passenger manifest. A Node.js/Express backend with a SQLite database handles matching and booking records, while Firebase provides Google Sign-In restricted to \texttt{@thapar.edu} accounts and phone-OTP verification for drivers. The result is a structured, visible replacement for today's wait-and-hope system, deliverable over a 12-week timeline using only free-tier tools.

\section{Introduction}
Campus Ride is proposed to digitize campus auto booking for both riders and drivers. It is a mobile application, built with Flutter so the same codebase runs on Android and the web, that lets students pick a pickup and drop stop and be automatically matched to an available auto, either one already on the road passing their stop, or the next one waiting in a queue, while drivers get a simple dashboard showing their current trip, passenger list, and queue position. The goal is to replace guesswork and informal waiting with a shared, visible booking system, the same way ride-hailing apps replaced hailing a cab on the street.
...
\section{Summary}
Getting a shared auto around Thapar campus currently means waiting at a stop and hoping one with a free seat passes by, with no visibility for students into wait times or for drivers into where demand is building. Campus Ride replaces this informal process with a two-sided mobile app, built in Flutter for both Android and web, that lets students book a seat by choosing a pickup and drop stop and automatically pools them onto an auto already en route in the right direction, or dispatches the next auto from a fair, cyclic idle queue if no such match exists. Drivers get a simple dashboard showing their queue position or live passenger manifest, and students see their driver's contact details, vehicle number, and UPI QR code once a booking is confirmed, so fares are fixed and disputes are eliminated. The system is built on a Node.js/Express backend with a SQLite database, using Firebase for Google Sign-In (restricted to \texttt{@thapar.edu} accounts) and driver phone-OTP verification. Over a 12-week timeline, the project moves from backend and authentication setup through the matching engine, driver and student interfaces, and end-to-end testing on both web and Android, using only free-tier tools and requiring no external funding.
