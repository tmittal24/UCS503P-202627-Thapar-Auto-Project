...
flowchart TD
    ...
    P1 -->|Registered Idle Auto| Driver
...
flowchart TD
    ...
    P3_3 -->|Save Pending Booking| DB_Bookings
...
The ER diagram below models the five core tables backing Thapar Auto: `STUDENT`,
`BOOKING`, `AUTO`, `DRIVER`, and `LOCATION`. A `BOOKING` links a `STUDENT` to the
`AUTO` that will carry them, and separately references `LOCATION` twice — once for
the pickup stop and once for the drop stop. Each `DRIVER` operates exactly one `AUTO`.

![Thapar Auto ER Diagram](er_diagram.png)

---

## 4. Activity Diagram — Booking & Dispatch Flow

The activity diagram below traces the full booking loop across three swimlanes —
**Student**, **System (Backend)**, and **Driver** — from stop selection through the
pooling/idle-queue decision, payment, and trip completion (which returns the auto to
the back of the idle queue).

![Thapar Auto Activity Diagram](activity_diagram.jpg)
